...
| Method | Hostname | Best for | Guide |
| --- | --- | --- | --- |
| **Stack API key** | `{stack-id}-es.logit.io` | Full programmatic access with the stack key | This page |
| **User credentials** | `{stack-id}-es-user.logit.io` | Per-user or service-account access (Teams roles) | [OpenSearch API (user credentials)](/opensearch-api/user-credentials) |
| **Hosted MCP** | `{stack-id}-mcp.logit.io` | Read-only Cursor / Claude MCP | [Hosted OpenSearch MCP](/opensearch-api/hosted-mcp) |

User credentials and hosted MCP require a **Business** or **Custom** plan and must be enabled per stack under **Settings → Access**.
...
- [Hosted OpenSearch MCP](/opensearch-api/hosted-mcp) — Cursor / Claude
