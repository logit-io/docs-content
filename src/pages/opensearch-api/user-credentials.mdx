...
Username is your **user ID** (humans) or `svc-{id}` (service accounts). Password is the value shown after enable / create / rotate.

```bash copy
curl -u "{userId}:{password}" \
  "https://{stack-id}-es-user.logit.io/_cluster/health?pretty"
```
...
curl -u "{userId}:{password}" \
  ...
  "https://{stack-id}-es-user.logit.io/{index}/_search?pretty" \
