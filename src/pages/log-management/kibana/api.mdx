...
You can use Basic Authentication to connect to the Logit.io Proxy and access Kibana without having to go 
...
You have the option to connect your application directly to the Logit.io API to do a range of things such as load dashboards, visualizations, index patterns and mappings etc. or load them straight from your computer.

This article explains how to call REST API Endpoints to access Kibana using our Logit.io proxy and connect to it using Basic 
Authentication.
...
<Callout type="info">
For account and stack management via API, see the [Developer API](/developer-api/overview).
</Callout>

Each request to a REST API Endpoint must include all of the necessary information for Kibana to fulfill the request. If all the necessary 
information is provided the API requests return JSON output.

Use the following operations to interact with the Kibana via the Logit.io Proxy.
...
### Example of a curl request to access your Logit.io Kibana instance
...
Both curl requests call the same API Endpoint. This request returns detailed information about your Kibana instance’s health including the 
resource usage, the server status, installed plugins, plugin statuses and metrics.

There are many other Kibana features that can be accessed via REST API through the Logit.io Proxy. These features are explained in the 
Elastic docs [here](https://www.elastic.co/guide/en/kibana/current/api.html)