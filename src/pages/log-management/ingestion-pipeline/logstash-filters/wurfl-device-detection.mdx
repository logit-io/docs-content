---
title: "Logstash Filter: WURFL Device Detection"
metaTitle: "Logstash WURFL Device Detection Filter Plugin Reference"
description: "Enriches events with device intelligence from WURFL. Includes usage guidance, configuration options, validation steps, and troubleshooting for the wurfl_device_detection filter plugin on Logit.io."
stackTypes: logs
---
...
2. In the `filter { ... }` section, add a `wurfl_device_detection` block.
...
- Confirm the `wurfl_device_detection` block compiles without syntax errors.
...
- GitHub package: [`logstash-filter-wurfl_device_detection`](https://github.com/logstash-plugins/logstash-filter-wurfl_device_detection)
