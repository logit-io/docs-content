---
title: "Logstash Filter: UUID"
metaTitle: "Logstash UUID Filter Plugin Reference"
description: "Adds UUID values to events. Includes usage guidance, configuration options, validation steps, and troubleshooting for the uuid filter plugin on Logit.io."
stackTypes: logs
---

# UUID filter plugin

Generates a random UUID string for each event. Use it to give every document a stable, unique identifier you can reference from alerts, dashboards, or dedupe jobs.
...
`uuid` is used in the Logstash filter stage. Adds UUID values to events.
...
2. In the `filter { ... }` section, add a `uuid` block.
...
- Confirm the `uuid` block compiles without syntax errors.
...
- GitHub package: [`logstash-filter-uuid`](https://github.com/logstash-plugins/logstash-filter-uuid)
