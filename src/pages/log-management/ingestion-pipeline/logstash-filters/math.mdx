---
title: "Logstash Filter: Math"
metaTitle: "Logstash Math Filter Plugin Reference"
description: "Applies arithmetic operations to numeric fields. Includes usage guidance, configuration options, validation steps, and troubleshooting for the math filter plugin on Logit.io."
stackTypes: logs
---

# Math filter plugin

Applies basic arithmetic operations to numeric fields. Use it to compute derived metrics (for example converting milliseconds to seconds) directly inside the pipeline. This plugin is not part of the default Logstash distribution; it is pre-installed on Logit stacks.

- Package: `logstash-filter-math`
- Coverage source: explicitly installed in the Logit image
- Official catalog entry: No

## Plugin overview

`math` is used in the Logstash filter stage. Applies arithmetic operations to numeric fields.
...
filter {
  math {
    calculate => [
      [ "[http][response][time_ms]", "/", "1000.0", "[http][response][time_seconds]" ]
    ]
  }
}
```
...
2. In the `filter { ... }` section, add a `math` block.
...
- Confirm the `math` block compiles without syntax errors.
...
- GitHub package: [`logstash-filter-math`](https://github.com/robin13/logstash-filter-math)
