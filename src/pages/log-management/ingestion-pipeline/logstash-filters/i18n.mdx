---
title: "Logstash Filter: i18n"
metaTitle: "Logstash i18n Filter Plugin Reference"
description: "Normalizes text by removing special characters. Includes usage guidance, configuration options, validation steps, and troubleshooting for the i18n filter plugin on Logit.io."
stackTypes: logs
---

# i18n filter plugin

Normalises non-ASCII text by transliterating accented characters into their closest ASCII equivalents. Useful when downstream systems expect a restricted character set.
...
`i18n` is used in the Logstash filter stage. Normalizes text by removing special characters.
...
- Important options: `transliterate`.
...
- `transliterate` (type: array; default: none) — List of field names whose values should be transliterated.
...
filter {
  i18n {
    transliterate => [ "[user][display_name]", "[city]" ]
  }
}
```
...
2. In the `filter { ... }` section, add a `i18n` block.
...
- Confirm the `i18n` block compiles without syntax errors.
...
- GitHub package: [`logstash-filter-i18n`](https://github.com/logstash-plugins/logstash-filter-i18n)
