---
title: ElastAlert 2 Full Reference
metaTitle: ElastAlert 2 alerting — Full Reference — Logit.io
description: Exhaustive single-page reference for ElastAlert 2 on Logit.io—every destination YAML option, alert subject/body rules, and shared formatting. For channel-specific setup and copy-paste examples, use Destinations and Creating alerts first.
stackTypes: logs
pagination: false
---

# ElastAlert 2 Full Reference

This page is the **Full Reference**: one long, exhaustive list of **ElastAlert 2** alert behaviour and **every** destination’s YAML keys, plus shared subject, body, and `$property$` rules. It complements—not replaces—the shorter guides with Logit.io examples: [Destinations](/log-management/alerting/destinations), [Subject & body](/log-management/alerting/alert-subject-and-body), and [Context & links](/log-management/alerting/alert-context-and-links).
...
E-mail subjects, Jira issue summaries, PagerDuty alerts, or any alerter that has a "subject" can be customized by adding an `alert_subject` that contains a custom summary.
...
Alerta alerter will post an alert in the Alerta server instance through the alert API endpoint. See [Alerta’s alert API reference](https://docs.alerta.io/api/reference.html#alerts) for the JSON format.
...
The following options use Python-like string syntax `{<field>}` or `%(<field>)s` to access parts of the match, similar to the CommandAlerter. For example: `Alert for {clientip}`. If the referenced key is not found in the match, it is replaced by the text indicated by the option `alert_missing_value`.
...
This alert type will send alerts to Alertmanager postAlerts. `alert_subject` and `alert_text` are passed as the annotations labeled `summary` and `description` accordingly. The labels can be changed. See [Prometheus Alertmanager alerts API](https://prometheus.io/docs/alerting/latest/alerts_api/) for the alert payload format.
...
Labels are sent either as static values or can be formatted using jinja2 templates that reference any field values from the Elastic record that triggered the alert.
...
alertmanager_fields: someLabel: "someElasticFieldName" someOtherLabel: "someOtherElasticFieldName"

Annotations are similar to labels where it can either be a static value or formatted using jinja2 templates. The only difference is that `alert_text` and `alert_subject` are merged back into the dictionary defined by `alertmanager_annotations` and are subjected to [different formatting rules](/log-management/alerting/alert-subject-and-body).
...
This alert will create a **Datadog event**. Events are limited to 4000 characters. If an event is sent that contains a message that is longer than 4000 characters, only the first 4000 characters will be displayed.
...
`dingtalk_sign`: DingTalk HMAC secret, used for message authentication. Configure the signing secret in your DingTalk custom bot security settings. Note that the algorithm provides authentication that *some* message was recently sent (within an hour) but does not authenticate the integrity of the current message itself.
...
`discord_emoji_title`: By default ElastAlert 2 will use the `:warning:` emoji when posting to the channel. You can use a different emoji per ElastAlert 2 rule. Use standard shortcodes (for example `:warning:`). If discord_embed_icon_url parameter is provided, emoji is ignored.
...
`email_from_field`: Use a field from the document that triggered the alert as the recipient. If the field cannot be found, the `email` value will be used as a default. Note that this field will not be available in every rule type, for example, if you have `use_count_query` or if it's `type: flatline`. You can optionally add a domain suffix to the field to generate the
...
Instructions for finding the SID and auth token associated with your account are in [Exotel’s developer documentation](https://developer.exotel.com/).
...
Route alerts into **Grafana OnCall** (Grafana IRM) using ElastAlert 2’s built-in integration. See [Grafana’s ElastAlert integration guide](https://grafana.com/docs/grafana-cloud/alerting-and-irm/irm/configure/integrations/integration-reference/oncall/elastalert/) for setup and payload details.
...
`http_post2_payload`: A JSON string or list of keys:values for the HTTP POST body. You can use `{{ field }}` (Jinja2) in keys and values to reference nested Elasticsearch fields and nested payload keys. If omitted, all match keys are sent. Ex: `"description_{{ my_field }}": "Type: {{ type }}\\nSubject: {{ title }}"`. When field names use dot notation or reserved characters, `_data` can access them. If `_data` clashes with your top-level data, set `jinja_root_name`.

`http_post2_raw_fields`: List of key:value pairs that define the POST body. Example: `ip: clientip` maps the Elasticsearch `clientip` field to JSON key `ip`. This field overwrites keys of the same name in `http_post2_payload`.

`http_post2_headers`: JSON string or list of key:value pairs to use as HTTP headers. You can use `{{ field }}` (Jinja2) in keys and values to reference nested fields in the match. Ex: `"Authorization": "{{ user }}"`. Headers `Content-Type: application/json` and `Accept: application/json;charset=utf-8` are set by default and may be overwritten. When field names use dot notation or reserved characters, `_data` can access them. If `_data` clashes with your top-level data, set `jinja_root_name`.

`http_post2_proxy`: Proxy URL if required. Only supports HTTPS.
...
Indexer alerter can be used to create a new alert in existing Opensearch/Elasticsearch. The alerter supports custom fields, and observables from the alert matches and rule data.
...
The Iris alerter can be used to create a new alert or case in Iris IRP System. The alerter supports adding tags, IOCs, and context from the alert matches and rule data.
...
A few words about `ioc_tlp_id` and `ioc_type_id`. `ioc_tlp_id` can be of three types: `1 - red`, `2 - amber`, `3 - green`. There are numerous values for `ioc_type_id`, and you can also add your custom ones. To find the ID for the type you are interested in, refer to your Iris instance's API at ''.

Complete YAML examples for IRIS and other alerters ship with the ElastAlert 2 distribution in its `examples` directory.
...
The ElastAlert 2 distribution includes a sample account file (`jira_acct.yaml` under `examples/rules/`). Your account file is YAML formatted.
...
> **Note:** **API migration:** From May 2025, Atlassian deprecated legacy JQL search endpoints. ElastAlert 2 uses the Jira client’s `enhanced_search_issues` method for bump ticket logic, which targets the newer Jira API internally. With a compatible Jira Python library (3.10.5+), this requires no rule changes.
...
See [Hookshot webhook handling](https://matrix-org.github.io/matrix-hookshot/latest/setup/webhooks.html#webhook-handling) for URL format and payload expectations.
...
`mattermost_webhook_url`: The webhook URL. Follow [Mattermost incoming webhooks](https://developers.mattermost.com/integrate/webhooks/incoming/) to create a webhook, then paste the URL here.
...
`mattermost_msg_fields`: You can add fields to your Mattermost alerts using this option. You can specify the title using `title` and the text value using `value`. Additionally you can specify whether this field should be a `short` field using `short: true`. If you set `args` and `value` is a formattable string, ElastAlert 2 will format the incident key based on the provided array of fields from the rule or match. See [Mattermost message attachment fields](https://developers.mattermost.com/integrate/reference/message-attachments/#fields) for layout options.
...
These options are specific to the PagerDuty V2 Events API; see [Send an event to PagerDuty](https://developer.pagerduty.com/api-reference/b3A6Mjc0ODI2Nw-send-an-event-to-pager-duty) for payload semantics when using `v2`.
...
For Squadcast-specific field names and webhook format, see [Squadcast’s ElastAlert integration guide](https://support.squadcast.com/integrations/alert-source-integrations-native/elastalert) and your service’s alert-source settings in the Squadcast UI.
...
`servicenow_proxy`: By default ElastAlert 2 does not use a network proxy to send notifications to ServiceNow. Set this option using `hostname:port` if you need a proxy. Only supports HTTPS.
...
`service_now_additional_fields`: Optional flat map of extra incident columns: each key is a ServiceNow **column name** (not the display label; custom fields often use a `u_` prefix such as `u_affected_user`), each value is a string, number, or boolean written to that field when the incident is created. Keys map directly to fields on the record. Use simple scalars unless your table’s API expects otherwise (complex or multi-value fields may need encoding per your instance’s REST rules). See the example below.
...
Additional columns beyond the required keys above go under `service_now_additional_fields` (see the optional key description). Always use the **column name**, not the display name.
...
`slack_webhook_url`: The webhook URL generated by your Slack App. Follow the [Slack incoming webhook quickstart](https://docs.slack.dev/messaging/sending-messages-using-incoming-webhooks) to create a new webhook. Note that the advanced, interactive Slack formatting options, using the Block Kit components, are not supported.
...
Official Webex incoming webhook documentation:
...
Match parameter names to the field list your Flashduty integration expects. See [Flashduty’s ElastAlert 2 integration guide](https://docs.flashcat.cloud/en/flashduty/elastalert2-integration-guide) for the full parameter reference.
