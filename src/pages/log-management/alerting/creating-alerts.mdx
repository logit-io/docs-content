...
description: Step-by-step guide to creating ElastAlert 2 YAML rules on Logit.io, testing them, and choosing rule types and notification destinations.
stackTypes: logs
---
...
<Callout type="info">
 See [Rule types](/log-management/alerting/rule-types) and [Destinations](/log-management/alerting/destinations) for full examples.
</Callout>
...
- Every rule has a `type:` (for example `frequency`, `spike`, `change`). See **[Rule types](/log-management/alerting/rule-types)** for one guide per type, each with a full YAML example.
- The `alert:` section chooses where notifications go (`email`, `slack`, `post`, `pagerduty`, and many more). See **[Destinations](/log-management/alerting/destinations)** for one guide per channel.
- To customize **subjects, bodies, top counts, and Discover links**, see **[Subject & body](/log-management/alerting/alert-subject-and-body)** and **[Context & links](/log-management/alerting/alert-context-and-links)**.

For troubleshooting validation and common mistakes, see [Check your ElastAlert rule](/log-management/alerting/guides/validating-alert-rules). Execution output from managed ElastAlert is also logged to your stack (**elastalert** index); see [Overview — ElastAlert execution logs](/log-management/alerting/overview#elastalert-execution-logs-in-your-stack).