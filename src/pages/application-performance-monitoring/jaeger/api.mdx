...
<Callout type="info">
  Basic Authentication is a simple authentication method in HTTP(S) where the
  client includes a username and password with each request. The username and
  password is combined with a colon in-between as a string. You don't need to
  worry about how to create a basic authentication header as we have done it for
  you.
</Callout>

<Callout type="info">
  For account and stack management via API, see the [Developer
...
Click [here](https://dashboard.logit.io/profile) to view your profile page.
