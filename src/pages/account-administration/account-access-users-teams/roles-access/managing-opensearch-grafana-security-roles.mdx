...
metaTitle: Learn How to Manage OpenSearch & Grafana Security Roles
...
<Callout type="info">
When **[OpenSearch API (user credentials)](/opensearch-api/user-credentials)** is enabled on a stack (Business / Custom), these Teams OpenSearch roles also apply to requests on `{stack-id}-es-user.logit.io`. Stack API key access on `{stack-id}-es.logit.io` is unchanged and is not limited by Teams roles.
</Callout>
