...
<Steps>
  ...
  **Comprehensive data collection:** This tool gathers logs, metrics, and tracing information, offering an all-encompassing platform for 
  ...
  Copy the configuration file below (making the above changes if necessary) and overwrite the contents of `otelcol-config-extras.yml` (this file can be found in the otelcollector folder where you installed the OpenTelemetry Collector demo application e.g. `opentelemetry-demo/src/otelcollector/otelcol-config-extras.yml`).
  ...
  exporters:
    ...
    otlp:
      ...
      endpoint: 'https://@opentelemetry.endpointAddress:strip_quotes:@opentelemetry.httpsPort:strip_quotes'
    logging:
      verbosity: detailed
  service:
    extensions:
