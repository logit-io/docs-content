...
<Steps>
  ...
  You will we now have a `node_modules` folder in addition to the `package.json` and a `package-lock.json` file.
  ...
  Open the `tracing.js` file with your choice of text editor, copy and past the following code into the file and then save.
  ...
  Next open the `app.js` file with your text editor, copy and past the following code into the file and then save.
  ...
  Open the `config.js` file using your text editor, copy and paste the following into the file and then save. Use the tab to choose between 
  ...
  You will see feedback from the app so that you know that is running, the message should say "Server is running on http://localhost:3000".
  ...
  If you open your browser and enter http://localhost:3000 you will see that action of browsing to the page served by the NodeJS app will 
  send further traces to your stack.
