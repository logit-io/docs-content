...
<Steps>
  ...
  ```erlang copy filename="otel_sample_app.erl" showLineNumbers /@opentelemetry.username/ /@opentelemetry.password/ /@opentelemetry.endpointAddress/ /@opentelemetry.httpsPort/
  ...
  create_sample_trace() ->
    ...
    AuthHeader = "Basic " ++ base64:encode_to_string(Username ++ ":" ++ Password),
    
      application:set_env(opentelemetry_exporter, otlp_endpoint, 
          os:getenv("OTLP_ENDPOINT", "https://" ++ Endpoint ++ ":" ++ Port)),
  ...
  ```erlang copy filename="sys.config" showLineNumbers
  [
      {opentelemetry, [
        {resource, [
  ...
  Run the following command in Terminal window or Command Prompt:
