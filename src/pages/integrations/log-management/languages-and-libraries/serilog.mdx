...
<Steps>
  ...
  <Callout type="info">
    This guide is based on an application created using an empty .NET Core MVC Web app template targeting .NET 8 in Visual Studio 2022 (VS).
    This guide was tested with the following versions of the packages: 
    Serilog.AspNetCore `8.0.3`, Serilog.Sinks.Network `2.0.2.68`, and Serilog.Extensions.Hosting `8.0.0`.
  </Callout>
  
  You will need to add the `Serilog` and `Serilog.Sinks.Network` NuGet packages via the command line 
  or the VS menu option :`Tools -> NuGet Package Manager -> Manage NuGet Packages for Solution`. 

  Powershell commands to install the required packages are as follows which can be entered in the Terminal window in VS:
  ```ps copy
...
Serilog is a popular logging library for [.NET](/integrations/log-management/languages-and-libraries/dotnet) and unlike many other logging libraries, 
...
files are preserved in the [JSON](/integrations/log-management/data-types/json) format.
...
either [NLog](/integrations/log-management/languages-and-libraries/nlog) or [log4net](/integrations/log-management/languages-and-libraries/log4net) for migrating your log data from .Net.
