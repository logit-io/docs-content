...
color: "#c02607"
stackTypes: logs, apm
description: Get started quickly with Ruby logging and OpenTelemetry using our configuration guide on how to send Ruby data to Logit.io
sslPortType: tcp-ssl
portType: tcp
...
<Steps>
  ...
  ### Simple Ruby Logging Example

  If you have your own Ruby app ready to send logs to Logit.io, you just need to install the `logstash-logger` gem 
  and make sure your logger object is set up with the correct `host` and `port` details.

  The simple Ruby script created below demonstrates how to send logs to Logit.io using the `logstash-logger` gem.
  
  Create a new directory for your project and call the new directory `logging_example`.

  Open a Terminal window or Command Prompt and navigate into the new `logging_example` directory. 
  ...
  This Ruby command will create a new `Gemfile` in the current directory. Open the `Gemfile` and replace the contents with the following:
  ```ruby copy
  ...
  Create a new Ruby file in the same directory and call it `logger_test.rb` and paste the following code into it:

  ```ruby copy filename="logger_test.rb" showLineNumbers /@logstash.host/ /"@logstash.sslPort:strip_quotes"/
  ...
  Install the `logstash-logger` gem by running the following command in your terminal:

  ```bash copy
  bundle install
  ```

  You can now run the Ruby script by executing the following command in your terminal:
